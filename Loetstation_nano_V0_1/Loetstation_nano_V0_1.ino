...
// variables control
  ...
  unsigned int temp_SOLL = 380; //eingestellte temperatur
 ...
 // conditions
  ...
  unsigned int temp_max = 9000; // max einstellbare Temperatur in grad Celcius
  int temp_min = 1;  // min einstellbare Temperatur in grad Celcius 
  int temp_delta = 20;
  int temp_standby = temp_min; // stand by temp
  int temp_counter = 10 ; // only update temp in intervalls of loops
  int temp_counter_i = 0 ; // loop counter
...
void loop() {
  ...
  if (temp_counter_i > temp_counter) {
...
// program ______ end _______________________________________________________________
  ...
  digitalWrite(led_gruen, HIGH);  delay(100);  digitalWrite(led_gruen, LOW);
   ...
   lcd.setCursor(10,0);   lcd.print(temp_SOLL);  lcd.print(" "); // to overwrite the old values
...
int temp_measure() {
  ...
  delay(50);
  double Temp = analogRead(temp_pin);  //read analog Value
